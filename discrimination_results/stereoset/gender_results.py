...
def extract_score(json_string):
    """
    Extract the "score" from a JSON-like string in the Gender columns.
    """
    try:
...
export_file = 'gender_final.csv'  # Replace with the correct path to your export.csv file
export_df = pd.read_csv(export_file)

# Extract scores from all Gender columns
gender_columns = [col for col in export_df.columns if "Gender" in col]
for col in gender_columns:
    export_df[f'score_{col}'] = export_df[col].apply(extract_score)

# Compute average score for gender columns that don't contain "ref"
for col in gender_columns:
    if "ref" not in col:
...
bias_file = 'bias_data/gender_bias.csv'
bias_df = pd.read_csv(bias_file)

bias_df = bias_df[bias_df['bias_type'] == 'gender']
...
for segment_name, segment_df in segments.items():
    ...
    threshold_results = {}
    ref_column = "Gender input + output ref"

    gender_columns = [
    col for col in gender_columns
    ...
    # Calculate gender column metrics
    gender_results = {}
    ...
    # Convert gender results to DataFrame
    gender_results_df = pd.DataFrame.from_dict(gender_results, orient='index')

    # Save results for this segment
    segmented_results[segment_name] = {
        "threshold_results": threshold_results_df,
        "gender_results": gender_results_df
    }
    i += 1
...
for segment_name, results in segmented_results.items():
    ...
    print("Gender Column Results:")
    print(results["gender_results"])